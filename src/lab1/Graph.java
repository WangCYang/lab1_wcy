...
public class Graph {
	...
	public void addEdge(String lhs, String rhs, int aCount) {
		...
		m_map.get(lhs).add(new Node(rhs, aCount));
		m_set.add(rhs);
	}
	...
	public void addEdge(String lhs, String rhs) {
		...
		m_map.get(lhs).add(new Node(rhs, 1));
		m_set.add(rhs);
	}
	...
	private LinkedList<String> dfs(String str, Map<String, Boolean> visited) {
	...
	private LinkedList<String> bfs(String str, Map<String, Boolean> visited) {
	...
	public Map<String, vertex> Dijkstra(String str) {
		...
		strToVtx.get(str).dist = 0;
		bhp.insert(strToVtx.get(str));
	...
	public Pair<LinkedList<LinkedList<String>>, Integer> getAllCloest(String src, String des) {
		...
		Stack<LinkedList<String>> stk = new Stack<>();
		...
		while (!stk.isEmpty()) {
			...
			if (fathers.size() != 0) {
				String father = fathers.getFirst();
				fathers.removeFirst();
				stk.add(fathers);
				path.add(father);
				if (father.equals(des)) {
					allPath.add((LinkedList<String>) path.clone());
					path.removeLast();
				}
